...
const IMAGES = {
  hero: {
    ...
    secondary:
      'https://images.unsplash.com/photo-1582139329536-e7284fece509?auto=format&fit=crop&w=2000&q=80', // Guard on duty
  },
  services: {
    guardias:
      'https://images.unsplash.com/photo-1590422915838-8e6d23485ce0?auto=format&fit=crop&w=800&q=80', // Security Guard
    patrullaje:
      'https://images.unsplash.com/photo-1548695602-9984e1b4db1b?auto=format&fit=crop&w=800&q=80', // Patrol Car
    alarmas:
      'https://images.unsplash.com/photo-1558002038-1091a5756131?auto=format&fit=crop&w=800&q=80', // Smart Home/Alarm
    guardpod:
      'https://plus.unsplash.com/premium_photo-1661962360677-22d765873730?auto=format&fit=crop&w=800&q=80', // Modern surveillance/tech
    drones:
      'https://images.unsplash.com/photo-1473968512647-3e447244af8f?auto=format&fit=crop&w=800&q=80', // Drone
    acceso:
      'https://images.unsplash.com/photo-1632213702844-1e0615781374?auto=format&fit=crop&w=800&q=80', // Access Control/Turnstile
  },
  solutions: {
    condominios:
      'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=800&q=80', // Apartment building
    mineria:
      'https://images.unsplash.com/photo-1581093588401-fbb07398e29c?auto=format&fit=crop&w=800&q=80', // Industry/Mining
    retail:
      'https://images.unsplash.com/photo-1534452203293-494d7ddbf7e0?auto=format&fit=crop&w=800&q=80', // Mall/Retail
    enventos:
  ...
  about:
    'https://images.unsplash.com/photo-1521737711867-e3b97375f902?auto=format&fit=crop&w=1200&q=80', // Team/Meeting
  blog: {
  ...
  handler: async (ctx) => {
    ...
    if (!existingConfig) {
      await ctx.db.insert('site_config', {
        ...
        address_main:
          'Av. Providencia 1234, Oficina 505, Providencia, Santiago',
        social_links: {
    ...
    const servicesData = [
      {
        ...
        description: 'Personal certificado OS10 con entrenamiento táctico.',
        icon: 'shield-check',
      ...
      },
      {
        ...
        description: 'Móviles de seguridad con monitoreo GPS 24/7.',
        icon: 'car',
      ...
      },
      {
        slug: 'alarmas-ajax',
        title: 'Alarmas Ajax',
        description: 'Tecnología de intrusión inalámbrica de grado 2.',
        icon: 'bell',
      ...
      },
      {
        slug: 'guardpod',
        title: 'GuardPod',
        description: 'Torres de vigilancia autónomas con IA.',
        icon: 'eye',
        image: IMAGES.services.guardpod,
      },
      {
        slug: 'drones-seguridad',
        title: 'Vigilancia Aérea',
        description: 'Drones con cámaras térmicas para grandes perímetros.',
        icon: 'drone',
        image: IMAGES.services.drones,
      },
      {
        slug: 'control-acceso',
        title: 'Control de Acceso',
        description: 'Gestión inteligente de entradas y salidas.',
        icon: 'lock',
        image: IMAGES.services.acceso,
      },
    ];
    ...
    const solutionsData = [
      {
        slug: 'condominios',
        title: 'Seguridad para Condominios',
        description: 'Protección integral para comunidades residenciales.',
        image: IMAGES.solutions.condominios,
      },
      {
        slug: 'mineria',
        title: 'Seguridad Minera e Industrial',
        description: 'Protocolos estrictos para faenas críticas.',
        image: IMAGES.solutions.mineria,
      },
      {
        slug: 'retail',
        title: 'Seguridad en Retail',
        description: 'Prevención de pérdidas y seguridad para clientes.',
        image: IMAGES.solutions.retail,
      },
      {
        slug: 'eventos',
        title: 'Seguridad de Eventos',
        description: 'Control de masas y protección VIP.',
        image: IMAGES.solutions.enventos,
      },
      {
        slug: 'construccion',
        title: 'Seguridad en Construcción',
        description: 'Vigilancia de obras y control de materiales.',
        image: IMAGES.solutions.construccion,
      },
    ];
    ...
    for (const sol of solutionsData) {
      const exists = await ctx.db
        .query('solutions')
        .withIndex('by_slug', (q) => q.eq('slug', sol.slug))
        .first();
      if (!exists) {
    ...
    const clientsData = [
      {
        ...
        quote: 'Seguridad impecable 24/7.',
        icon: 'building',
      },
      {
        name: 'Kavak',
        ...
        quote: 'Control de acceso eficiente.',
        icon: 'car',
      },
      {
        name: 'Cencosud',
    ...
    for (const client of clientsData) {
      const exists = await ctx.db
        ...
        .filter((q) => q.eq(q.field('name'), client.name))
        .first();
      if (!exists) {
    ...
    const seedBlocks = async (pageSlug: string, blocks: any[]) => {
      const existing = await ctx.db
        ...
        .withIndex('by_page_order', (q) => q.eq('page_slug', pageSlug))
        .collect();
      for (const block of existing) {
        await ctx.db.delete(block._id);
      }
      for (const block of blocks) {
        await ctx.db.insert('content_blocks', {
          ...block,
          page_slug: pageSlug,
          is_visible: true,
        });
    ...
    await seedPage(
      '/',
      'Home',
      'Guardman Chile - Seguridad Privada Experta',
      'Líderes en seguridad OS10 y tecnología en Chile.'
    );
    await seedBlocks('/', [
      {
        type: 'hero_video',
        ...
        subtitle:
          'Guardias certificados, tecnología de vanguardia y protección integral sin contratos forzosos',
        data: {
      ...
      },
      {
        ...
        data: {
          ...
          features: [
            {
              ...
              description: 'Operativo en 24 horas',
            },
            {
              icon: 'signal',
              title: 'Conectividad 4G/5G',
              description: 'Internet satelital incluido',
            },
            {
              icon: 'map-pin',
              title: 'Fácil Despliegue',
              description: 'Cualquier ubicación remota',
            },
            {
              icon: 'shield-check',
              title: 'Blindaje Nivel 3',
              description: 'Protección certificada',
            },
          ],
          stats: [
          ...
          ctas: [
            {
              text: 'Solicitar GuardPod',
              href: '/servicios/guardpod',
              variant: 'accent',
            },
            {
              text: 'WhatsApp',
              href: 'https://wa.me/56930000010',
              variant: 'whatsapp',
            },
          ],
        },
    ...
    await seedPage(
      '/servicios',
      'Servicios',
      'Servicios de Seguridad - Guardman Chile',
      'Explora nuestra gama completa de servicios de seguridad.'
    );
    ...
    await seedPage(
      '/soluciones',
      'Soluciones',
      'Soluciones por Industria - Guardman Chile',
      'Seguridad adaptada a su industria.'
    );
    ...
    const existingCerts = await ctx.db
      .query('partners')
      .filter((q) => q.eq(q.field('type'), 'certification'))
      .collect();
    if (existingCerts.length === 0) {
      await ctx.db.insert('partners', {
        name: 'OS10 Carabineros',
        logo_url: '/logos/os10.png',
        type: 'certification',
        order: 1,
      });
      await ctx.db.insert('partners', {
        name: 'ISO 9001',
        logo_url: '/logos/iso9001.png',
        type: 'certification',
        order: 2,
      });
    ...
    const blogPosts = [
      {
        ...
        excerpt:
          'Descubre cómo la inteligencia artificial y la vigilancia aérea están revolucionando la protección de activos en Chile.',
        ...
        content: [
          {
            type: 'text',
            content:
              'La seguridad privada está experimentando una transformación radical gracias a la incorporación de nuevas tecnologías.',
          },
          { type: 'h2', content: 'Vigilancia Aérea Autónoma' },
          {
            type: 'image',
            content: IMAGES.services.drones,
            alt: 'Drone vigilancia',
            caption: 'Drones patrullando.',
          },
          {
            type: 'text',
            content:
              'Los drones permiten cubrir grandes extensiones de terreno en minutos.',
          },
          {
            type: 'video',
            content: 'https://www.youtube.com/watch?v=J5jZ15z-r_g',
            caption: 'Demo drones.',
          },
        ],
      },
        ...
        content: [
          {
            type: 'text',
            content:
              'La certificación OS10 es la garantía de personal capacitado.',
          },
          {
            type: 'list',
            content: '',
            items: ['Legislación', 'Prevención', 'Emergencias'],
          },
        ],
      },
      {
        ...
        content: [
          {
            type: 'text',
            content: 'El verano es crítico para la seguridad del hogar.',
          },
    ...
    for (const industry of industriesData) {
      const exists = await ctx.db
        .query('industries')
        .withIndex('by_slug', (q) => q.eq('slug', industry.slug))
        .first();
      if (!exists) {
    ...
    // 9. Seed Process Steps for Servicios page
    const processStepsData = [
      {
        page_slug: 'servicios',
        number: 1,
        title: 'Diagnóstico',
        description: 'Análisis de riesgos y evaluación de vulnerabilidades',
        order: 1,
      },
      {
        page_slug: 'servicios',
        number: 2,
        title: 'Propuesta',
        description: 'Diseño de solución personalizada a medida',
        order: 2,
      },
      {
        page_slug: 'servicios',
        ...
        description: 'Despliegue operativo y configuración',
        order: 3,
      },
      {
        page_slug: 'servicios',
        number: 4,
        title: 'Monitoreo',
        description: 'Supervisión continua y soporte 24/7',
        order: 4,
      },
    ];
    ...
    for (const step of processStepsData) {
      const exists = await ctx.db
        .query('process_steps')
        .withIndex('by_page_slug', (q) => q.eq('page_slug', step.page_slug))
        .filter((q) => q.eq(q.field('number'), step.number))
        .first();
      if (!exists) {
        await ctx.db.insert('process_steps', step);
      }
    }
